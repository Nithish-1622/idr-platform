...
def run_vta01a_pipeline():
    data_root = r"e:\idr-platform\ml\data"
    s_path = r"Synchronised V abd S datasets\Categorised IOVNB Dataset\Vta (Driver E)\Vta01a\S-Vta1a.csv"
    v_path = r"Synchronised V abd S datasets\Categorised IOVNB Dataset\Vta (Driver E)\Vta01a\V-Vta1a.csv"
    ...
    print("CURRENT DATASET: Vta01a (Driver E)")
    ...
    print("Checkpoint saved: YES")
    
    # Export ONNX
    onnx_path = r"e:\idr-platform\ml\models\exports\deep_idr_model_S3a.onnx"
    os.makedirs(os.path.dirname(onnx_path), exist_ok=True)
    model.eval()
    try:
        import sys
        # Save stdout and override it so torch.onnx doesn't crash on windows console with emojis
        old_stdout = sys.stdout
        with open(os.devnull, 'w') as f:
            sys.stdout = f
            torch.onnx.export(model, torch.randn(1, 10, 3).to(device), onnx_path, input_names=['input'], output_names=['output'])
        sys.stdout = old_stdout
        print(f"ONNX exported: YES")
    except Exception as e:
        sys.stdout = old_stdout
        print(f"ONNX exported: NO ({e})")
    
    print("\n--- 3. Inference & Evaluation (on Full Vta01a dataset) ---")
    model.eval()
    with torch.no_grad():
    ...
    plot_path = os.path.join(os.path.dirname(__file__), "..", "..", "..", "docs", "ml", "trajectory_Vta01a.png")
