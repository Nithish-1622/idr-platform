...
def run_y1_pipeline():
    ...
    s_path = r"Synchronised V abd S datasets\Categorised IOVNB Dataset\Y (Driver D)\Y1\S-Y1.csv"
    v_path = r"Synchronised V abd S datasets\Categorised IOVNB Dataset\Y (Driver D)\Y1\V-Y1.csv"
    ...
    print("CURRENT DATASET: Y1 (Driver D)")
    ...
    print("\n--- 3. Inference & Evaluation (on Full Y1 dataset) ---")
    model.eval()
    with torch.no_grad():
    ...
    plot_path = os.path.join(os.path.dirname(__file__), "..", "..", "..", "docs", "ml", "trajectory_Y1.png")
