...
def run_s3c_pipeline():
    data_root = r"e:\idr-platform\ml\data"
    s_path = r"Synchronised V abd S datasets\Categorised IOVNB Dataset\S (Driver A)\S3c\S-S3c.csv"
    v_path = r"Synchronised V abd S datasets\Categorised IOVNB Dataset\S (Driver A)\S3c\V-S3c.csv"
    ...
    print("\n--- 3. Inference & Evaluation (on Full S3c dataset) ---")
    model.eval()
    with torch.no_grad():
    ...
    plot_path = os.path.join(os.path.dirname(__file__), "..", "..", "..", "docs", "ml", "trajectory_S3c.png")
